...
NOTE: If you prefer to use https://k3s.io/[k3s] instead of OpenShift, just switch to the https://github.com/vshn/k8up-tutorial/tree/k3d[k3d branch] in the tutorial repository. The https://github.com/vshn/k8up-tutorial[master branch] is based on https://github.com/kubernetes/minikube[Minikube].
...
. Find the URL to the WordPress deployment using the `oc get routes` command.
...
[start=2]
. Find the URL to the Minio deployment using the `oc get routes` command.
